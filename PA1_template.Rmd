...
```
The mean and median total number of steps taken each day are `r mean` and `r median`, respectively.
...
```
The `r max` 5-minute interval, on average, contains the maximum number of steps.
...
```
The mean and median total number of steps taken each day after imputation are `r new_mean` and `r new_median`, respectively. These values differ from the previous estimates. The estimates increase in magnitude and become more normalized after imputation.
